...
class RTMPClient
{
	...
	{
		$op = new RtmpOperation(new RtmpMessage("connect",(object)array(
					"app" => $this->application,
					"flashVer" => "MAC 11,1,106,62",
					"swfUrl" => "",
	...
	private function handle_invoke(RtmpPacket $p)
	{
		...
		else
		{
			...
			is_callable($h) && is_array($op->getResponse()->arguments) && call_user_func_array($h, $op->getResponse()->arguments);
			$op->clearResponse();
